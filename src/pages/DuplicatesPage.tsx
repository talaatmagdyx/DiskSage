...
export function DuplicatesPage() {
  ...
  return (
    <div className="mx-auto max-w-6xl px-8 py-8">
      ...
      {(duplicate.progress || scanning) && <Card className="mt-6 overflow-hidden"><div className="flex items-center justify-between border-b border-line px-6 py-4"><div><p className="text-xs uppercase tracking-[0.14em] text-muted">Duplicate scan</p><h2 className="mt-1 font-semibold capitalize">{phaseLabel(duplicate.summary?.phase ?? duplicate.progress?.phase ?? "discovering")}</h2></div>{scanning && <LoaderCircle className="animate-spin text-sage-300" size={20} />}</div><div className="grid grid-cols-5 gap-px bg-line"><Metric label="Files" value={(duplicate.summary?.filesScanned ?? duplicate.progress?.filesScanned ?? 0).toLocaleString()} /><Metric label="Candidates" value={(duplicate.summary?.candidateFiles ?? duplicate.progress?.candidateFiles ?? 0).toLocaleString()} /><Metric label="Hashed" value={formatBytes(duplicate.summary?.bytesHashed ?? duplicate.progress?.bytesHashed ?? 0)} /><Metric label="Groups" value={(duplicate.summary?.groupsFound ?? duplicate.progress?.groupsFound ?? 0).toLocaleString()} /><Metric label="Wasted" value={formatBytes(duplicate.summary?.reclaimableBytes ?? duplicate.progress?.reclaimableBytes ?? 0)} /></div><p className="truncate px-6 py-3 font-mono text-xs text-muted" aria-live="polite">{duplicate.summary ? "Duplicate scan state saved locally." : duplicate.progress?.currentPath ?? "Preparing the next stage…"}</p></Card>}
      ...
      {duplicate.summary?.phase === "completed" && orderedGroups.length === 0 ? <Card className="mt-6 grid min-h-56 place-items-center text-center"><div><Files className="mx-auto text-muted" size={34} /><h2 className="mt-4 font-semibold">No content-identical groups found</h2><p className="mt-2 text-sm text-muted">Same-size files with different content were excluded.</p></div></Card> : orderedGroups.length > 0 ? <VirtualizedList className="mt-6 space-y-4 pb-24" items={orderedGroups} itemKey={(group) => group.id} estimateSize={(group) => 155 + group.copies.length * 70} label="Verified duplicate groups" renderItem={(group, index) => {
